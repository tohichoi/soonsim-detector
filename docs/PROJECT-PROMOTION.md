...
![배변판 영역(ROI) 캘리브레이션 화면 — 4개 코너의 픽셀 좌표가 라벨로 표시됨](assets/roi_overlay.jpg)

*4개 코너(`P1(180,90)` ~ `P4(200,300)`)의 픽셀 좌표를 지정하면, 해당 폴리곤 내부만 감시 대상이 됩니다.*
...
- Mike 디렉터 + ATD 에이전트 팀: 20개 단위·통합 테스트 100% 통과, 무결점 상용 수준 완성
```
...
| **프로그램 품질** | 개발자 개인 실력에 의존, 잔고장 위험 | 20개 단위·통합 테스트 및 품질 감사 통과 | **상용 프로그램 수준 안정성** |
